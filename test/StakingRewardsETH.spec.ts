...
import { stakingRewardsETHFixture } from './fixtures'
import { REWARDS_DURATION, expandTo18Decimals, mineBlock, getApprovalDigest } from './utils'

import StakingRewardsETH from '../build/StakingRewardsETH.json'

chai.use(solidity)

describe('StakingRewardsETH', () => {
  ...
  beforeEach(async () => {
    const fixture = await loadFixture(stakingRewardsETHFixture)
  ...
  it('deploy cost', async () => {
    const stakingRewards = await deployContract(wallet, StakingRewardsETH, [
      wallet.address,
